...
The tray's contents don't survive a page reload on their own — there's no server and no database, so
"saving between sessions" means writing real files to a real folder on disk, via the **File System
...
**Scope note.** Folder sync now restores both the *avatar library* (which images exist) and the
*board layout* (which room each one was standing in) — see "Board layout" above. Both are gated on a
connected folder; with none connected, neither survives a reload. See section 10.
...
**Board-layout persistence requires a connected folder**, exactly like the avatar library does
(section 5, "Board layout") — with no folder connected, layout still doesn't survive a reload, since
there's nothing to write it to. Switching to a *different* folder mid-session (clicking "Folder: …"
again while already connected) isn't specifically handled beyond what section 5 already describes —
items from the previous folder stay in state until removed.
